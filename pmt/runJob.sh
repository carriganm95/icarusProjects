...
if ! {
    ...
    #source /pkgs/conda/etc/profile.d/conda.sh
    #conda activate py39 || { echo "Failed to activate py39"; exit 15; }
    ...
    # Make sure we have ifdhc
    #setup ifdhc || { echo "Failed to setup IFDH"; exit 15; }
}; then
...
if ! setup icaruscode v10_06_00_04p02 -q e26:prof; then
    echo "ERROR: setup icaruscode failed" >&2
    cleanup_and_exit 30
fi
if ! setup sbnana v10_01_01 -q e26:prof; then
    echo "ERROR: setup sbnana failed" >&2
    cleanup_and_exit 31
else
    ups active
...
echo "Running ${macro_file} on job ${jobNum} with output ${outputFile}"

calDir="${INPUT_TAR_DIR_LOCAL}/nashome/m/micarrig/scratch/calibrationdb/"

pushd "${work_dir}" >/dev/null
echo "Executing: python3 ${macro_file} ${jobNum} ${work_dir}/${outputFile} ${calDir}"
if ! python3 ${macro_file} ${jobNum} ${work_dir}/${outputFile} ${calDir}; then
    popd >/dev/null
